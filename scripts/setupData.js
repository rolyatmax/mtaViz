...
mta.parseAndSaveData = function() {
	...
	function init(turnstileURL) {
		...
		var rand = _.random( urls.length - 1 );
		turnstileURL = turnstileURL || urls[rand];
	...
	function addDownload() {
		var dataString = JSON.stringify( mta.data.turnstile );
		var blob = new Blob([ dataString ], {type: 'text/json'});
		var filename = 'turnstile.json';
		var href = window.webkitURL.createObjectURL( blob );
